...
const SearchHistory: React.FC<ISearchHistory> = ({ onWeatherSearch }) => {
  ...
  return (
    <div className="bg-white/20 dark:bg-[#1A1A1A4D] py-5 px-4 rounded-[20px] mt-10">
      ...
      {isEmpty(history) ? (
        ...
        history?.map((item) => (
          <div
            key={item.id}
            className="flex flex-row justify-between items-center text-xs md:text-base bg-white/40 shadow-md rounded-[20px] px-4 py-5 dark:bg-[#1A1A1A80] mb-3"
          >
            ...
            <div className="flex flex-row gap-2 items-center">
              <button
                className="bg-white dark:bg-[#1A1A1A80] hover:bg-gray-100 dark:hover:bg-[#454545] rounded-full p-2 text-[#93919e] cursor-pointer"
                onClick={() =>
                  onWeatherSearch({
